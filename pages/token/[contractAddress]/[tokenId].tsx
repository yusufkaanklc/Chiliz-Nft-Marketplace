...
export default function TokenPage({ nft, contractMetadata }: Props) {
    ...
    async function buyListing() {
        ...
        if (directListing?.[0]) {
            ...
            throw new Error("No listing found for this NFT");
        }
                ...
                <Stack spacing={"20px"}>
                    {contractMetadata && (
                        <Flex alignItems={"center"}>
                            <Box borderRadius={"5px"} overflow={"hidden"} mr={"10px"}>
                                <MediaRenderer
                    ...
                    <Box mx={2.5}>
                        ...
                        <Link href={`/profile/${nft.owner}`}>
                            <Flex direction={"row"} alignItems={"center"}>
                                <Avatar src="https://bit.ly/broken-link" h={"25px"} w={"25px"} mr={"10px"}></Avatar>
                                <Text fontSize={"small"}>{nft.owner.slice(0, 6)}...{nft.owner.slice(-4)}</Text>
                            </Flex>
                        </Link>
                    </Box>
                    <Stack backgroundColor={"transparent"} p={2.5} borderRadius={"5px"}>
...
export const getStaticProps: GetStaticProps = async (context) => {
    ...
    const sdk = new ThirdwebSDK("spicy-chain")
